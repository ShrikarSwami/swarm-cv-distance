...
Usage:
    blender --background --python test_sweep_tall_pillar.py
"""
...
OUTPUT_DIR = Path(_project_root) / "dataset_smoke_test" / "sweep_tall_pillar"
...
# ===== MAIN =====
print("=" * 70)
print("EXPOSURE SWEEP — TALL PILLAR METHOD")
print("=" * 70)

# CLEAR sweep
print("\n--- CLEAR (kloofendal_43d_clear_puresky, azimuth=45°) ---")
print("Expected shadow: LEFT (west)")
print(f"{'Strength':>10} | {'Center':>24} | {'BG':>24} | {'Diff':>8} | {'Time':>6}")
print("-" * 85)

clear_results = []
for s in [0.15, 0.1, 0.08, 0.05, 0.03, 0.02]:
    ...
    print(f"{s:>10.3f} | {c_str:>24} | {b_str:>24} | {diff:>8.2f} | {t:>5.2f}s")
    clear_results.append((s, center, bg, diff, t))

# Find best clear
best_clear_s = max(clear_results, key=lambda x: x[3] if x[3] < 250 else 0)[0]
...
# DUSK sweep
print("\n--- DUSK (belfast_sunset_puresky, azimuth=90°) ---")
print("Expected shadow: LEFT (west)")
print(f"{'Strength':>10} | {'Center':>24} | {'BG':>24} | {'Diff':>8} | {'Time':>6}")
print("-" * 85)

dusk_results = []
for s in [0.3, 0.2, 0.15, 0.1, 0.08, 0.05]:
...
# Find best dusk
best_dusk_s = max(dusk_results, key=lambda x: x[3] if x[3] < 250 else 0)[0]
...
print("SUMMARY")
print("=" * 70)
print(f"Clear:  strength={best_clear_s}, diff={max(r[3] for r in clear_results if r[3] < 250):.2f}")
print(f"Dusk:   strength={best_dusk_s}, diff={max(r[3] for r in dusk_results if r[3] < 250):.2f}")
